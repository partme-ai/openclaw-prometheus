...
| --- | --- | --- | --- |
| Frontier (leader) | `gpt-5.2` | high | Primary leader/orchestrator for planning, coordination, and frontier-class reasoning. |
...
| `analyst` | `gpt-5.2` | medium | Requirements clarity, acceptance criteria, hidden constraints (frontier-orchestrator, frontier) |
| `planner` | `gpt-5.2` | medium | Task sequencing, execution plans, risk flags (frontier-orchestrator, frontier) |
| `architect` | `gpt-5.2` | high | System design, boundaries, interfaces, long-horizon tradeoffs (frontier-orchestrator, frontier) |
| `debugger` | `gpt-5.4-mini` | high | Root-cause analysis, regression isolation, failure diagnosis (deep-worker, standard) |
| `executor` | `gpt-5.2` | high | Code implementation, refactoring, feature work (deep-worker, standard) |
| `team-executor` | `gpt-5.2` | medium | Supervised team execution for conservative delivery lanes (deep-worker, frontier) |
...
| `security-reviewer` | `gpt-5.2` | medium | Vulnerabilities, trust boundaries, authn/authz (frontier-orchestrator, frontier) |
| `performance-reviewer` | `gpt-5.4-mini` | medium | Hotspots, complexity, memory/latency optimization (frontier-orchestrator, standard) |
| `code-reviewer` | `gpt-5.2` | high | Comprehensive review across all concerns (frontier-orchestrator, frontier) |
| `dependency-expert` | `gpt-5.4-mini` | high | External SDK/API/package evaluation (frontier-orchestrator, standard) |
| `test-engineer` | `gpt-5.2` | medium | Test strategy, coverage, flaky-test hardening (deep-worker, frontier) |
...
| `code-simplifier` | `gpt-5.2` | high | Simplifies recently modified code for clarity and consistency without changing behavior (deep-worker, frontier) |
...
| `critic` | `gpt-5.2` | high | Plan/design critical challenge and review (frontier-orchestrator, frontier) |
| `vision` | `gpt-5.2` | low | Image/screenshot/diagram analysis (fast-lane, frontier) |
<!-- OMX:MODELS:END -->
...
<!-- OMX:RUNTIME:START -->
<session_context>
**Session:** omx-1776592965583-dk9ole | 2026-04-19T10:02:46.477Z

**Codebase Map:**
  src/: collect-cache, index.register.test, openclaw-sdk.d, plugin-config.test, plugin-config, scrape-auth.test, scrape-auth, types, version
  src/collectors/: channels, cron, health, models, nodes, presence, runtime, sessions, skills, usage
  src/formatters/: json, prometheus.test, prometheus
  scripts/: test-metrics-client
